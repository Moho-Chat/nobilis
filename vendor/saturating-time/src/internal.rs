    ...
    F: Fn(&T, Duration) -> Option<T>,
{
    ...
    loop {
        ...
        match next {
            ...
            // is a FILETIME with 100ns granularity, so subtracting 1ns
            // returns Some(unchanged) rather than None. The loop below only
            // shrinks `step` when it sees None, so once step reached 1ns the
            // None branch became unreachable, `res` stopped moving, and this
            // spun on one core forever.
            //
            // A step too small to change the value has found the limit for
            // this type's resolution just as surely as one that overflows, so
            // it is treated the same: shrink, and stop when there is nothing
            // left to shrink.
            Some(_) => {
                if step == ONE_NS {
                    return res;
