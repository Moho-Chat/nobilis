...
fn snowflake_at(unix_secs: i64) -> String {
...
fn resolve_person(state: &AppState, account_id: &str, name: &str) -> Result<String> {
...
fn resolve_channel(state: &AppState, account_id: &str, name: &str) -> Option<String> {
...
fn message_summary(state: &AppState, buffer_id: &str, msg: &Value, cfg: &crate::accounts::DiscordAccountConfig) -> Value {
    let author = &msg["author"];
    let from = author["global_name"]
        .as_str()
        .filter(|s| !s.is_empty())
